...
// По формуле: I=m/(h*h)
    Console.WriteLine("Индекс массы тела: " + mass/(height*height));
    Console.ReadLine();
}
...
// Задание №3.а) Программа подсчета расстояния между точками
{
    ...
    Console.WriteLine($"Расстояние между точками составит: {result:F2}");
    Console.ReadLine();
...
// Задание №3.б) Программа подсчета расстояния между точками через метод
    ...
    Console.WriteLine($"Расстояние между точками составит: {Method(x1, y1, x2, y2):F2}");
    Console.ReadLine();

// Проверка работоспособности метода с другими числами:
    Console.WriteLine($"Расстояние между точками составит: {Method(2, 3, 5, 6.789):F2}");
    Console.ReadLine();
}
...
/* СТАВЛЮ В КОММЕНТАРИИ, ТАК КАК PUBLIC (= способ доступа, который позволяет либо переменную или метод использовать вне класса) ВЛИЯЕТ НА ПРЕДЫДУЩИЕ КЕЙСЫ
